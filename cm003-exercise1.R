...
## 2. Assign it to `preamble`. valid name can't start with  _ number
...
class(commute)
(n <- length(commute))
## Get the mean commute time via the `sum` function and `n`. Store in `mu`. can't do average when type is character 
mu <- sum(commute) /n
mu
